<x-layout>
    <div class="min-h-screen flex bg-gray-100">
        ...
        <main class="flex-1 p-4 md:p-10 min-w-0">
            <div class="mb-4">
                <a href="{{ route('admin.users') }}" class="text-green-600 hover:underline text-sm">&larr; Back to
                    Users</a>
            </div>
                                                    ...
                                                    <tr class="border-b hover:bg-gray-50">
                                                        <td class="p-3">
                                                            @if($report->image)
                                                                <td class="p-3">
                                                                    @if($report->image)
                                                                        <img src="{{ $report->image }}" class="w-16 h-16 object-cover rounded">
                                                                    @else
                                                                        <span class="text-gray-400 text-sm">No Image</span>
                                                                    @endif
                                                                </td> class="w-16 h-16 object-cover rounded">
                                                            @else
                                                            <span class="text-gray-400 text-sm">No Image</span>
                                                        @endif
                                                        </td>
